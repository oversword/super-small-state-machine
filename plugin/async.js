import S, { Node, noop , Stack, Trace, InterruptGotoNode, Return, Changes, MaxIterationsError } from "../index.js"

export const Wait = Symbol("SSSM Wait")
export class WaitNode extends Node {
	static type = Wait
	static typeof(object, objectType) { return object === Wait }
	static proceed = InterruptGotoNode.proceed
}
...
export function runAsync(...input) {
	...
	const executor = async () => {
		...
		while (r < iterations) {
			...
			while (interruptionStack.length && currentState[Uninterruptable] <= 0)
				currentState = await S._perform(this, currentState, interruptionStack.pop())
			const action = await S._execute(this, currentState)
			if (action === Wait && !interruptionStack.length) {
				await new Promise(waitForInterruption)
				// Perform one interruption to make sure we don't exit
				currentState = await S._perform(this, currentState, interruptionStack.pop())
			}
